<template>
  <q-form @submit="salvar()" class="q-pa-none q-ma-none">
    <div class="q-xl">
      ...
      <div class="row q-pa-sm q-sm">
        <div class="col-xs-12">
          <q-input class="q-pa-xs" outlined v-model="endereco.complemento" label="Complemento" />
        </div>
      </div>
    </div>

    <div class="q-pa-none q-mx-sm" style="text-align: right;">
      <q-btn label="Finalizar" @click="salvar" class="add-row" color="blue-8" text-color="white" />
    </div>
  </q-form>
</template>
    ...
    salvar() {
      if (this.endereco.cep > 0) {
        ...
        this.$q.notify({
          ...
          color: "black",
          avatar: "https://findicons.com/files/icons/990/vistaico_toolbar/256/symbol_error.png"
        });
      }
    },
    async consultar() {
      ...
      if (result.status == 200 && !result.data.erro) {
        ...
        this.endereco.lagradouro = data.logradouro;
