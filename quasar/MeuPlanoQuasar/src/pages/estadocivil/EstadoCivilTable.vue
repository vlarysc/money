    ...
    <div class="q-pa-none q-gutter-sm" style="text-align: right;">
      <q-form>
        ...
        >
          <q-card
            class="q-ma-md"
            style="width:400px;height:400px; border-radius: 15px;"
          >
            <q-toolbar>
              <q-header></q-header>
              <q-toolbar-title
...
export default {
  components: { EstadoCivilForm },
  computed: {
    ...mapState("estadocivil", ["estadocivils"])
  },
  async mounted() {
  ...
  methods: {
    ...
    async onRequest(props) {
      ...
      await this.getEstadoCivils(pageSize);

      this.myPagination = {
        ...this.myPagination,
        ...pagination,
        rowsNumber: this.estadocivils.total
      };
      this.loading = false;
    },
    ...
    async salvarEstadoCivils(estadocivil) {
      await this.postEstadoCivil(estadocivil);
      this.onRequest();
      this.toolbar = false;
    },
    async atualizar(estadocivil) {
      await this.putEstadoCivil({
        id: estadocivil.id,
        payload: estadocivil
      });
      this.onRequest();
