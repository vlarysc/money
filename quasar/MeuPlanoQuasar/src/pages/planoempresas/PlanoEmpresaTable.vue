...
export default {
  ...
  computed: {
    ...mapState("planoempresa", ["planoempresas"]),
  ...
  methods: {
    ...mapActions("planoempresa", ["getPlanoEmpresas"]),
    ...
    search() {
      this.$router.push("/planoempresas/" + this.documento1);
    },
    adicionar() {
      Loading.show();
      this.$router.push(
        `/planoempresas/new?page=${this.options.page}&size=${this.options.size}`
      );
      Loading.hide();
    },
      ...
      this.$swal
        ...
        .then(async result => {
          if (result.isConfirmed) {
            this.loading = true;
            await this.deletePlanoEmpresa(id);
            await this.getPlanoEmpresas(this.options);
            this.loading = false;
          }
        });
    },
    update(planoempresa) {
      ...
      this.$router.push(
        `/planoempresas/${planoempresa.pessoa.documento1}?page=${this.options.page}&size=${this.options.size}`
      );
    },
    ruleDocument1(val) {
      if (val.length == 11) {
