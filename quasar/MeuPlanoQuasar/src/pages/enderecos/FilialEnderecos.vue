<template>
  <div class="q-pa-xs">
    <q-table
      :grid="$q.screen.xs"
      :visible-grid="['option']"
      :title="`Filial: ${filial.pessoa.nome}`"
      flat
      bordered
      dense
      class="bg-white"
            ...
            <div class="row" @click="update(props.row)">
              <div class="col-6">
                <q-card-section class="text-left">
                  Endereço
                  <br />
                  <strong>{{ props.row.logradouro }}</strong>
                </q-card-section>
              </div>
              <div class="col-6">
                <q-card-section class="text-left">
...
import FilialEnderecoForm from "./FilialEnderecoForm";

export default {
  components: { FilialEnderecoForm },
  computed: {
  ...
  methods: {
    ...mapActions("filial", ["addEndereco", "deleteEndereco"]),
