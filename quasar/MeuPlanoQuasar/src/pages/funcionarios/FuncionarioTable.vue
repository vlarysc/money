...
export default {
  ...
  methods: {
    ...mapActions("funcionario", ["getFuncionarios"]),
    ...
    search() {
      this.$router.push("/funcionarios/" + this.documento1);
    },
    adicionar() {
      Loading.show();
      this.$router.push(
        `/funcionarios/new?page=${this.options.page}&size=${this.options.size}`
      );
      Loading.hide();
    },
      ...
      this.$swal
        ...
        .then(async result => {
          if (result.isConfirmed) {
            ...
            await this.getFuncionarios(this.options);
            this.loading = false;
          }
        });
    },
    update(funcionario) {
      ...
      this.$router.push(
        `/funcionarios/${funcionario.pessoa.documento1}?page=${this.options.page}&size=${this.options.size}`
      );
    },
    ruleDocument1(val) {
      if (val.length == 11) {
