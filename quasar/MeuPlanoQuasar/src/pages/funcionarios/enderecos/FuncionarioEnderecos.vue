<template>
  <div class="q-pa-xs">
    <q-table
      :grid="$q.screen.xs"
      :visible-grid="['option']"
      :title="`Funcionario: ${funcionario.pessoa.nome}`"
      flat
      bordered
      dense
      class="bg-white"
      color="black"
      binary-state-sort
      :data="funcionario.pessoa.enderecos"
      :columns="columns"
      row-key="name"
    >
...
import FuncionarioEnderecoForm from "./FuncionarioEnderecoForm";

export default {
  components: { FuncionarioEnderecoForm },
  computed: {
  ...
  methods: {
    ...mapActions("funcionario", ["addEndereco", "deleteEndereco"]),
