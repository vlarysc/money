    ...
    <div class="q-pa-none q-gutter-sm" style="text-align: right;">
      <q-form>
        ...
        >
          <q-card
            class="q-ma-md"
            style="width:400px;height:408px; border-radius: 15px;"
          >
            <q-toolbar>
              <q-header></q-header>
              <q-toolbar-title
            ...
            <q-card-section>
              <ReligioesForm
                :newReligiao="religiao"
                @atualizar="atualizar"
                @salvar="salvarReligioes"
              />
            </q-card-section>
          </q-card>
        </q-dialog>
...
import ReligioesForm from "./ReligioesForm";
import { pageSizeDefault } from "src/utils/newDatas";

export default {
  components: { ReligioesForm },
  computed: {
    ...mapState("religiao", ["religioes"])
  },
  async mounted() {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          label: "Observações",
          align: "left",
          field: row => row.observacoes.substring(0, 120)
        },
  ...
  methods: {
    ...
    async onRequest(props) {
      ...
      await this.getReligioes(pageSize);

      this.myPagination = {
        ...this.myPagination,
        ...pagination,
        rowsNumber: this.religioes.total
      };
      this.loading = false;
    },
    novoReligiao() {
    ...
    async salvarReligioes(religiao) {
      await this.postReligiao(religiao);
      this.onRequest();
      this.toolbar = false;
    },
    async atualizar(religiao) {
      await this.putReligiao({ id: religiao.id, payload: religiao });
      this.onRequest();
      this.toolbar = false;
    },
